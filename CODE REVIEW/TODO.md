...
- [~] **S-W02 (P1) (S)** Closure-Local Functions Make Six Controls Inoperative — _CLAUDE H-18, H-20, H-21, M-24; FULL/COPILOT F-19; INDEPENDENT R-16_
  - **In progress — PR #315 `website-handlers-and-clamps`** (2026-09-14): the six handlers are exported to `window` after their definitions; `TankAlarm-112025-Server-BluesOpta/check_web_pages.py` (node --check per script block + inline-handler reachability) runs as the `check-web-pages` CI job and flagged exactly these six before the fix. Also clamps Sample Minutes to 1092 and momentary relay durations to 65535 (client `uint16_t` limits, CLAUDE M-06/M-08).
- [~] **S-W03 (P1) (S)** Calibration Identity, Units, and Input Validation — _FULL/COPILOT F-03; INDEPENDENT R-07; CLAUDE H-19, M-60_
  - **In progress — PR #316** (2026-09-14): `parseSensorKey()` splits at the last colon with a 0–255 integer suffix at all three sites (submit used to POST `clientUid:"dev", sensorIndex:1` for `dev:…:0`); POST/DELETE `/api/calibration` use `is<uint8_t>()` / `is<float>()` presence checks so sensor index 0 and a verified level of 0 in are accepted, require a finite non-negative level, and resolve the client/sensor pair before any work (404 otherwise).
