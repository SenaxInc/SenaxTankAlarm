...
- [~] **S-W02 (P1) (S)** Closure-Local Functions Make Six Controls Inoperative — _CLAUDE H-18, H-20, H-21, M-24; FULL/COPILOT F-19; INDEPENDENT R-16_
  - **In progress — PR #315 `website-handlers-and-clamps`** (2026-09-14): the six handlers are exported to `window` after their definitions; `TankAlarm-112025-Server-BluesOpta/check_web_pages.py` (node --check per script block + inline-handler reachability) runs as the `check-web-pages` CI job and flagged exactly these six before the fix; after review it tokenises regex literals after keywords, matches any `on<event>` attribute, only counts `window.fn =` assignments that execute on load, fails closed on a missing node/sketch/page, carries a `--selftest`, and gates `build-firmware`. Also clamps Sample Minutes to 1092 and momentary relay durations to 65535 (client `uint16_t` limits, CLAUDE M-06/M-08).
