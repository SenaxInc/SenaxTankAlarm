...
- **Removed stuck outboard DFU enable:** Dropped the boot-time `card.dfu {"name":"stm32","off":true}` initialization and the RS-485 quiescing handshake that depended on unrouted hardware.
- **Documentation:** Corrected DFU notes to reflect that ODFU is not supported on this carrier without a hardware change (Notecarrier CX/F or hand-wired `ALT_DFU` lines); IAP remains the supported OTA path.
