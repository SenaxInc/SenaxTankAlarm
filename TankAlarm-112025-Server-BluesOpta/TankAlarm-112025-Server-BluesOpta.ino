...
struct ServerConfig {
  ...
  char githubRouteAlias[32]; // Notehub Route Alias for proxying GitHub API calls (proxy-only cellular)
  char smsPrimary[20];
...
struct SensorRecord {
  ...
  double previousLevelEpoch;  // When the previous value was recorded
...
struct SensorCalibration {
  ...
  // Implied fluid specific gravity, derived from learnedSlope vs the theoretical
  // slope-for-water computed from the sensor's configured pressure range. Useful as
  // a sanity check ("operator says diesel, math says 0.86 \u2014 looks right") and as a
  // contamination/product-change indicator over time. 0.0f = not yet computed.
  float impliedSpecificGravity;
  // |impliedSG - presetSG|; flagged in the dashboard when > 0.05.
  float specificGravityDriftFromPreset;
};
...
// v1.9.22 autonomous Ethernet recovery (the Opta LAN8742 PHY often fails to relink after a
// soft reset from a USB-DFU flash or NVIC_SystemReset, historically needing a manual re-flash
// or power cycle).
...
window.validatePowerCombination=function(){
  ...
  const bv=parseInt(document.getElementById('batteryVoltage').value,10)||12;
  const vmon=document.getElementById('voltageMonitor').value||'none';
  const vin=vmon==='vin-divider';
  const rs485=vmon==='rs485';
  ...
  if(ps==='solar_modbus_mppt'&&bt==='none'){
    ...
    block=true;
  }else if(ps==='solar_mppt'&&bt==='none'){
    msg='MPPT controllers require a battery load. Switch to plain Solar (Passive Charger) for solar-direct installs.';
    block=true;
  }else if(ps==='solar_modbus_mppt'&&!rs485){
    msg='Modbus MPPT charger is selected as Power Source — set Voltage Monitor to RS485 so the firmware reads the SunSaver over Modbus. None / Voltage Divider will leave the charge controller unmonitored.';
  }else if(ps.startsWith('solar')&&bt==='none'&&vmon==='none'){
    msg='Solar-direct without any voltage monitoring cannot detect sunset. Strongly recommended to enable Voltage Divider or RS485.';
  }else if(ps==='grid'&&bt==='none'&&vin){
    msg='Voltage Divider on a grid-only install with no battery has nothing useful to read.';
  ...
  if(bv===24&&vin){
    ...
    const maxV=10.0/(r2/(r1+r2));
    if(maxV<27.0){
      const sep=msg?' ':'';
      msg+=sep+'24V pack with R1='+r1+'k, R2='+r2+'k caps at '+maxV.toFixed(1)+'V. Recommend R1=47k, R2=33k (max ~33V).';
    }
  }
...
window.updatePowerConfigInfo=function(){
  ...
  const vmon=document.getElementById('voltageMonitor').value||'none';
  const vin=vmon==='vin-divider';
  /* Hide battery voltage selector when no battery */
  const bvField=document.getElementById('batteryVoltageField');
  if(bvField)bvField.style.display=(bt==='none'||bt==='lipo')?'none':'flex';
  /* Hardware-requirement banner */
  const i=document.getElementById('powerConfigInfo');
  if(i){
    if(ps==='solar_modbus_mppt'){i.style.display='block';i.innerHTML='<strong>Hardware Requirement:</strong> Modbus MPPT requires Opta WiFi (AFX00002) or Opta RS485 (AFX00001). Opta Lite (AFX00003) needs an external RS485 transceiver.';}
    else if(ps==='solar_mppt'){i.style.display='block';i.innerHTML='<strong>Hardware Requirement:</strong> Use this for higher-quality non-monitored solar chargers. For basic PWM chargers (for example SunKeeper-6), use Solar (Passive Charger). This mode does not require RS485 and uses battery/vin telemetry only.';}
    else{i.style.display='none';}
  }
  /* Vin divider config section */
  const v=document.getElementById('vinConfigSection');if(v)v.style.display=vin?'block':'none';if(vin)updateVinCalc();
  /* Solar-only (no battery) section auto-enables when battery=None and power source is solar */
  const isSolarOnly=(bt==='none'&&ps.startsWith('solar'));
  const so=document.getElementById('solarOnlyConfigSection');if(so)so.style.display=isSolarOnly?'block':'none';
  const vinFields=['solarOnlyDebounceVField','solarOnlyDebounceSField','solarOnlySensorGateField','solarOnlySunsetVField'];
  vinFields.forEach(fid=>{const el=document.getElementById(fid);if(el)el.style.display=(isSolarOnly&&vin)?'flex':'none';});
  const warmupField=document.getElementById('solarOnlyWarmupField');
  if(warmupField)warmupField.style.display=(isSolarOnly&&!vin)?'flex':'none';
  /* Battery failure fallback only meaningful when there IS a battery on solar */
...
window.updateLoopPowerVisibility=function(id){const card=document.getElementById(`sensor-${id}`);if(!card)return;const lpSelect=card.querySelector('.loop-power-enabled');if(!lpSelect)return;const lpVal=lpSelect.value;const type=parseInt(card.querySelector('.sensor-type').value);const isCl=type===2;const powered=isCl&&lpVal!=='false';card.querySelector('.loop-power-warmup-field').style.display=powered?'flex':'none';card.querySelector('.loop-power-sample-delay-field').style.display=powered?'flex':'none';card.querySelector('.loop-power-pwm-channel-field').style.display=(isCl&&lpVal==='pwm')?'flex':'none';const smvf=card.querySelector('.sensor-min-voltage-field');if(smvf)smvf.style.display=powered?'flex':'none';const warn=card.querySelector('.loop-power-warning');if(warn)warn.style.display=isCl?'block':'none';updateLoopRangeAdvisory(id);};
window.updateLoopRangeAdvisory=function(id){const card=document.getElementById(`sensor-${id}`);if(!card)return;const adv=card.querySelector('.loop-power-range-advisory');if(!adv)return;const typeEl=card.querySelector('.sensor-type');const lpEl=card.querySelector('.loop-power-enabled');if(!typeEl||!lpEl){adv.style.display='none';return;}const type=parseInt(typeEl.value);const lpVal=lpEl.value;if(type!==2||lpVal==='false'){adv.style.display='none';return;}const vminEl=card.querySelector('.sensor-min-voltage');const vmin=parseFloat(vminEl?vminEl.value:'10')||10;const vsup=(lpVal==='dac')?11.0:12.0;const supTxt=(lpVal==='dac')?'the 11V DAC supply':'a 12.0V battery (conservative planning value)';const clipMa=Math.max(0,Math.min(25,(vsup-vmin)*10));const rMin=parseFloat(card.querySelector('.sensor-range-min').value)||0;const rMax=parseFloat(card.querySelector('.sensor-range-max').value)||0;const unit=card.querySelector('.sensor-range-unit').value||'';adv.style.display='block';if(clipMa>=20.25){adv.style.background='#d4edda';adv.style.border='1px solid #28a745';adv.innerHTML='<strong>Usable range check:</strong> with '+supTxt+' and a '+vmin+'V-minimum sensor, the full 4-20mA span is available - the entire '+rMin+'-'+rMax+' '+unit+' range can be measured without clipping.';return;}if(clipMa<=4.25){adv.style.background='#f8d7da';adv.style.border='1px solid #dc3545';adv.innerHTML='<strong>Usable range check:</strong> with '+supTxt+' a '+vmin+'V-minimum sensor cannot even sustain the 4mA live-zero - readings will not be reliable. Use a higher-voltage supply mode or a lower-minimum-voltage sensor.';return;}const frac=Math.max(0,(clipMa-4)/16);adv.style.background='#fff3cd';adv.style.border='1px solid #e0a800';let html='<strong>Usable range check:</strong> with '+supTxt+' and a '+vmin+'V-minimum sensor, the loop clips at ~'+clipMa.toFixed(1)+'mA = <strong>'+(frac*100).toFixed(0)+'% of the 4-20mA span</strong>.';if(rMax>rMin){const usable=rMin+frac*(rMax-rMin);const biggerMax=rMax*2;const biggerUsable=rMin+frac*(biggerMax-rMin);html+=' Readings above ~<strong>'+usable.toFixed(1)+' '+unit+'</strong> (of the '+rMin+'-'+rMax+' '+unit+' range) may be silently clipped. Tip: installing a larger-range sensor keeps working pressures in the un-clipped lower span - e.g. a '+rMin+'-'+biggerMax+' '+unit+' sensor is trustworthy up to ~'+biggerUsable.toFixed(1)+' '+unit+'.';}adv.innerHTML=html;};
window.updateSensorTypeFields=function(id){const card=document.getElementById(`sensor-${id}`);const type=parseInt(card.querySelector('.sensor-type').value);const digitalInfoBox=card.querySelector('.digital-sensor-info');const currentLoopInfoBox=card.querySelector('.current-loop-sensor-info');const alarmThresholdsGrid=card.querySelector('.alarm-thresholds-grid');const digitalAlarmGrid=card.querySelector('.digital-alarm-grid');const alarmSectionTitle=card.querySelector('.alarm-section-title');const pulsesPerRevField=card.querySelector('.pulses-per-rev-field');const switchModeField=card.querySelector('.switch-mode-field');const currentLoopTypeField=card.querySelector('.current-loop-type-field');const sensorMountHeightField=card.querySelector('.sensor-mount-height-field');const sensorRangeField=card.querySelector('.sensor-range-field');const analogVoltageField=card.querySelector('.analog-voltage-field');const loopPowerEnabledField=card.querySelector('.loop-power-enabled-field');if(type===0){digitalInfoBox.style.display='block';currentLoopInfoBox.style.display='none';switchModeField.style.display='flex';currentLoopTypeField.style.display='none';sensorMountHeightField.style.display='none';sensorRangeField.style.display='none';analogVoltageField.style.display='none';loopPowerEnabledField.style.display='none';card.querySelector('.loop-power-warmup-field').style.display='none';card.querySelector('.loop-power-sample-delay-field').style.display='none';card.querySelector('.loop-power-pwm-channel-field').style.display='none';const smv0=card.querySelector('.sensor-min-voltage-field');if(smv0)smv0.style.display='none';const lpw0=card.querySelector('.loop-power-warning');if(lpw0)lpw0.style.display='none';const adv0=card.querySelector('.loop-power-range-advisory');if(adv0)adv0.style.display='none';alarmThresholdsGrid.style.display='none';digitalAlarmGrid.style.display='grid';alarmSectionTitle.textContent='Float Switch Alarm';pulsesPerRevField.style.display='none';}else if(type===2){const monitorType=card.querySelector('.monitor-type').value;digitalInfoBox.style.display='none';currentLoopInfoBox.style.display='block';switchModeField.style.display='none';analogVoltageField.style.display='none';loopPowerEnabledField.style.display='flex';updateLoopPowerVisibility(id);const loopTypeSelect=card.querySelector('.current-loop-type');if(monitorType==='tank'){currentLoopTypeField.style.display='flex';loopTypeSelect.innerHTML='<option value="pressure">Pressure Sensor(Bottom-Mounted)</option><option value="ultrasonic">Ultrasonic Sensor(Top-Mounted)</option>';sensorMountHeightField.style.display='flex';sensorRangeField.style.display='flex';}else if(monitorType==='gas'){currentLoopTypeField.style.display='none';loopTypeSelect.innerHTML='<option value="pressure">Pressure Sensor</option>';loopTypeSelect.value='pressure';sensorMountHeightField.style.display='none';sensorRangeField.style.display='flex';}else{currentLoopTypeField.style.display='flex';loopTypeSelect.innerHTML='<option value="pressure">Pressure Sensor</option>';loopTypeSelect.value='pressure';sensorMountHeightField.style.display='none';sensorRangeField.style.display='flex';}alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';alarmSectionTitle.textContent='Alarm Thresholds';pulsesPerRevField.style.display='none';updateCurrentLoopFields(id);}else if(type===3){digitalInfoBox.style.display='none';currentLoopInfoBox.style.display='none';switchModeField.style.display='none';currentLoopTypeField.style.display='none';sensorMountHeightField.style.display='none';sensorRangeField.style.display='none';analogVoltageField.style.display='none';loopPowerEnabledField.style.display='none';card.querySelector('.loop-power-warmup-field').style.display='none';card.querySelector('.loop-power-sample-delay-field').style.display='none';card.querySelector('.loop-power-pwm-channel-field').style.display='none';const smv3=card.querySelector('.sensor-min-voltage-field');if(smv3)smv3.style.display='none';const lpw3=card.querySelector('.loop-power-warning');if(lpw3)lpw3.style.display='none';const adv3=card.querySelector('.loop-power-range-advisory');if(adv3)adv3.style.display='none';alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';alarmSectionTitle.textContent='Alarm Thresholds';pulsesPerRevField.style.display='flex';}else{const monitorType=card.querySelector('.monitor-type').value;digitalInfoBox.style.display='none';currentLoopInfoBox.style.display='none';switchModeField.style.display='none';currentLoopTypeField.style.display='none';analogVoltageField.style.display='flex';sensorRangeField.style.display='flex';loopPowerEnabledField.style.display='none';card.querySelector('.loop-power-warmup-field').style.display='none';card.querySelector('.loop-power-sample-delay-field').style.display='none';card.querySelector('.loop-power-pwm-channel-field').style.display='none';const smvA=card.querySelector('.sensor-min-voltage-field');if(smvA)smvA.style.display='none';const lpwA=card.querySelector('.loop-power-warning');if(lpwA)lpwA.style.display='none';const advA=card.querySelector('.loop-power-range-advisory');if(advA)advA.style.display='none';if(monitorType==='tank'){sensorMountHeightField.style.display='flex';}else{sensorMountHeightField.style.display='none';}alarmThresholdsGrid.style.display='grid';digitalAlarmGrid.style.display='none';alarmSectionTitle.textContent='Alarm Thresholds';pulsesPerRevField.style.display='none';}updateSensorGuidance(id);updateFluidVisibility(id);};
...
const addSensorBtn=document.getElementById('addSensorBtn');if(addSensorBtn)addSensorBtn.addEventListener('click',addSensor);
/* INPUTS */
...
const downloadBtn=document.getElementById('downloadBtn');if(downloadBtn)downloadBtn.addEventListener('click',()=>{const cfg=collectConfig();if(!cfg)return;const blob=new Blob([JSON.stringify(cfg,null,2)],{type:'application/json'});const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='client_config.json';document.body.appendChild(a);a.click();document.body.removeChild(a);URL.revokeObjectURL(url);});
/* LOAD CONFIG */
...
let btSel='agm';let bvSel=12;let vmonSel='none';if(c.solarCharger&&c.solarCharger.enabled)vmonSel='rs485';else if(c.vinMonitor&&c.vinMonitor.enabled)vmonSel='vin-divider';if(c.voltageMonitor==='none'||c.voltageMonitor==='vin-divider'||c.voltageMonitor==='rs485')vmonSel=c.voltageMonitor;
...
const importInput=document.getElementById('importFileInput');const importBtn=document.getElementById('importBtn');if(importBtn&&importInput){importBtn.addEventListener('click',()=>importInput.click());importInput.addEventListener('change',(e)=>{const f=e.target.files[0];if(!f)return;const r=new FileReader();r.onload=(evt)=>{try{loadConfig(JSON.parse(evt.target.result));}catch(err){showToast('Invalid JSON',true);}};r.readAsText(f);});}
...
const state={clients:[],sites:{},refreshing:false,timer:null,uiRefreshS:DEFAULT_REFRESH_S,paused:false,expandedDot:null,sparkData:{},sparkLoaded:false,sparkError:false};
...
function timeAgo(epoch){if(!epoch)return'never';const s=Math.max(0,Date.now()/1000-epoch);if(s<120)return'just now';if(s<3600)return Math.floor(s/60)+'m ago';return Math.floor(s/3600)+'h ago';}
...
try{const ac=new AbortController();const tid=setTimeout(()=>ac.abort(),10000);const res=await fetch('/api/history?days=90',{signal:ac.signal});clearTimeout(tid);if(!res.ok){state.sparkLoaded=true;state.sparkError=true;renderSparklines();return;}const data=await res.json();
const map={};(data.sensors||[]).forEach(t=>{const key=(t.client||'')+'|'+(t.sensorIndex||1);map[key]=t.readings||[];});
state.sparkData=map;state.sparkLoaded=true;state.sparkError=false;renderSparklines();}catch(e){console.warn('Sparkline data unavailable:',e.message);state.sparkLoaded=true;state.sparkError=true;renderSparklines();}}
...
static void handleFtpTestPost(EthernetClient &client, const String &body);
static void handleFtpDiscoverFingerprintPost(EthernetClient &client, const String &body);
...
static bool checkGitHubReleaseForTarget(uint8_t target,
  ...
  // When a Notehub Route Alias is configured, proxy the GitHub API call through it (useful on
  // proxy-only cellular networks). The route handles the host; we only supply the path via
  // "name". Otherwise fall back to a direct absolute "url".
...
void loop() {
  ...
  if (now - gLastLinkCheckMillis > 5000UL) {
    ...
    // Track how long the link has been continuously down (drives the recovery below).
    if (linkUp) {
      gLinkDownSinceMs = 0;
  ...
  // v1.9.22: autonomous Ethernet recovery. On sustained link-down, first re-run the bring-up
  // (re-begin) — this alone recovers the common post-DFU soft-latch case that used to need a
  // manual re-flash. Only if repeated re-begins fail, AND the link worked earlier this boot (so an
  // unplugged cable / absent DHCP server never boot-loops the unit), AND we are past the boot grace
  // (so the MCUboot health marker has had time to be written), escalate to a soft reset. The
  // Notecard poll path runs independently, so client telemetry is still processed meanwhile.
  if (gLinkDownSinceMs != 0 && Ethernet.linkStatus() != LinkON) {
    ...
    if (downFor > ETH_RELINK_GRACE_MS && (now - gLastEthRecoverMs) > ETH_RECOVER_INTERVAL_MS) {
      ...
      gEthRecoverAttempts++;
      Serial.print(F("Ethernet down "));
      Serial.print(downFor / 1000UL);
      Serial.print(F("s - recovery attempt "));
      Serial.println(gEthRecoverAttempts);
...
static bool ftpBackupClientConfigs(FtpSession &session, bool useFtps, char *error, size_t errorSize, uint8_t &uploadedFiles) {
  ...
  // Plain FTP and FTPS both issue MKD; 550 (already exists) is non-fatal.
  {
    char clientsDir[192];
    buildRemotePath(clientsDir, sizeof(clientsDir), "clients");
    bool mkdOk = useFtps
        ? gFtpsClient.mkd(clientsDir, error, errorSize)
        : ftpMakeDir(session, clientsDir, error, errorSize);
    if (!mkdOk) {
...
static bool archiveMonthToFtp(uint16_t year, uint8_t month) {
  ...
  // Ensure parent directory exists. MKD on an existing dir returns 550 which
  // ftpMakeDir/gFtpsClient.mkd both report; we treat that as non-fatal so
  // subsequent runs (after the first successful MKD) continue uploading.
  {
    char historyDir[192];
    snprintf(historyDir, sizeof(historyDir), "%s/history", gConfig.ftpPath);
    char mkdErr[128];
    mkdErr[0] = '\0';
    bool mkdOk = useFtps
        ? gFtpsClient.mkd(historyDir, mkdErr, sizeof(mkdErr))
        : ftpMakeDir(session, historyDir, mkdErr, sizeof(mkdErr));
    if (!mkdOk) {
      Serial.print(F("FTP archive: MKD history warning (non-fatal): "));
...
static void handleWebRequests() {
  ...
  } else if (method == "POST" && path == "/api/ftp-test") {
    if (contentLength > 512) {
      respondStatus(client, 413, "Payload Too Large");
    } else {
      handleFtpTestPost(client, body);
    }
  } else if (method == "POST" && path == "/api/ftp-discover-fingerprint") {
    if (contentLength > 512) {
      respondStatus(client, 413, "Payload Too Large");
    } else {
      handleFtpDiscoverFingerprintPost(client, body);
    }
...
static void sendClientDataJson(EthernetClient &client, const String &query) {
  ...
  for (uint8_t i = 0; i < gSensorRecordCount; ++i) {
    ...
    if (rec.previousLevelEpoch > 0.0) {
      sensorObj["d"] = rec.currentValue - rec.previousValue;
    }
...
// Probe the FTP/FTPS server with the SAVED settings: connect+login, upload a
// tiny test file, retrieve it back, and verify the content matches. Surfaces
// per-step diagnostics (with timings + per-step error messages) so the
// operator can see exactly which stage failed — DNS/connect, login, TLS
// handshake, PASV, STOR, or RETR.
//
// The test file is `<ftpPath>/<serverUid>/tankalarm-conn-test.txt`, overwritten
// on every test so it never accumulates. Operator can delete it manually if
// they want a clean directory; leaving it is harmless.
static void handleFtpTestPost(EthernetClient &client, const String &body) {
  JsonDocument doc;
  if (deserializeJson(doc, body)) {
    respondStatus(client, 400, F("Invalid JSON"));
  ...
  JsonDocument resp;
  resp["host"] = gConfig.ftpHost;
  resp["port"] = gConfig.ftpPort;
  resp["path"] = gConfig.ftpPath;
  resp["ftps"] = gConfig.ftpsEnabled;

  // Reject obvious config gaps before any blocking network work.
  if (!gConfig.ftpEnabled) {
    resp["ok"] = false;
    resp["error"] = "FTP is disabled — enable it in settings and Save first.";
    String json; serializeJson(resp, json);
    respondJson(client, json, 400);
    return;
  }
  if (gConfig.ftpHost[0] == '\0') {
    resp["ok"] = false;
    resp["error"] = "FTP Host is empty — fill it in and Save first.";
    String json; serializeJson(resp, json);
    respondJson(client, json, 400);
    return;
  }
  if (gConfig.ftpUser[0] == '\0') {
    resp["ok"] = false;
    resp["error"] = "FTP User is empty — fill it in and Save first.";
    String json; serializeJson(resp, json);
    respondJson(client, json, 400);
    return;
  }
  ...
  char remotePath[192];
  buildRemotePath(remotePath, sizeof(remotePath), "tankalarm-conn-test.txt");
  resp["testFile"] = remotePath;

  double nowEpoch = currentEpoch();
  char payload[160];
  int payloadLen = snprintf(payload, sizeof(payload),
      "TankAlarm FTP connection test\nfw=%s\nepoch=%.0f\nbuild=%d\n",
      FIRMWARE_VERSION, nowEpoch, (int)FIRMWARE_BUILD_SEQ);
  if (payloadLen <= 0) payloadLen = 0;

  JsonArray steps = resp["steps"].to<JsonArray>();
  char errBuf[160];
  errBuf[0] = '\0';
  ...
  if (connected) {
    ...
    st["step"] = "upload";
    st["ok"] = stored;
    st["bytes"] = (uint32_t)payloadLen;
    st["elapsedMs"] = (uint32_t)(millis() - t);
    if (!stored && errBuf[0]) st["error"] = errBuf;
  }

  // ---- Step 3: download (RETR) ----
  ...
  if (stored) {
    ...
    st["step"] = "download";
    st["ok"] = retrieved;
    st["bytes"] = (uint32_t)readBackLen;
    st["elapsedMs"] = (uint32_t)(millis() - t);
    if (!retrieved && errBuf[0]) st["error"] = errBuf;
  }

  // ---- Step 4: verify content matches ----
  bool matches = false;
  if (retrieved) {
    matches = (readBackLen == (size_t)payloadLen) &&
              (memcmp(readBack, payload, (size_t)payloadLen) == 0);
    JsonObject st = steps.add<JsonObject>();
    st["step"] = "verify";
    st["ok"] = matches;
    if (!matches) {
      char detail[64];
      snprintf(detail, sizeof(detail), "read-back %u bytes, expected %u",
               (unsigned)readBackLen, (unsigned)payloadLen);
      st["error"] = detail;
    }
  }
  ...
  gBackupInProgress = false;
  gFtpBackupCriticalPathActive = false;
  setFtpBackupStage("test-done");

  const bool ok = connected && stored && retrieved && matches;
  resp["ok"] = ok;
  resp["elapsedMs"] = (uint32_t)(millis() - testT0);
  if (ok) {
    char msg[96];
    snprintf(msg, sizeof(msg),
             "OK — wrote %d bytes and verified read-back%s",
             payloadLen, useFtps ? " (FTPS)" : "");
    resp["message"] = msg;
  } else {
    resp["error"] = (errBuf[0] != '\0') ? errBuf : "FTP test failed — see steps for details.";
  }
  resp["trace"] = gFtpsTraceBuf;

  Serial.print(F("/api/ftp-test: done ok="));
  Serial.print(ok ? 1 : 0);
  Serial.print(F(" elapsed_ms="));
  Serial.println(millis() - testT0);

  if (!client.connected()) {
    setFtpBackupStage("test-client-gone");
    clearFtpBackupMarker();
    return;
  }
  String json; serializeJson(resp, json);
...
// Discover the SHA-256 fingerprint of the FTPS server's TLS certificate so
// the operator can pin it. Connects TCP, sends AUTH TLS, runs the TLS
// handshake WITHOUT validating the certificate, captures the fingerprint,
// and closes. Requires FtpsClient::discoverFingerprint (FTPSclientOPTA
// >= 0.3.0). Uses gConfig.ftpHost/ftpPort as defaults; the request body
// can override either field with {"host":..., "port":..., "sni":...}.
static void handleFtpDiscoverFingerprintPost(EthernetClient &client, const String &body) {
  JsonDocument doc;
  if (deserializeJson(doc, body)) {
    respondStatus(client, 400, F("Invalid JSON"));
  ...
  const char *host = doc["host"].is<const char *>() && doc["host"].as<const char *>() && doc["host"].as<const char *>()[0] != '\0'
      ? doc["host"].as<const char *>()
      : gConfig.ftpHost;
  uint16_t port = doc["port"].is<uint16_t>() && doc["port"].as<uint16_t>() != 0
      ? doc["port"].as<uint16_t>()
      : gConfig.ftpPort;
  const char *sni = doc["sni"].is<const char *>() && doc["sni"].as<const char *>() && doc["sni"].as<const char *>()[0] != '\0'
      ? doc["sni"].as<const char *>()
      : (gConfig.ftpsTlsServerName[0] != '\0' ? gConfig.ftpsTlsServerName : nullptr);
  ...
  if (!host || host[0] == '\0') {
    resp["ok"] = false;
    resp["error"] = "FTP Host is empty — fill it in (no Save required) and try again.";
    String json; serializeJson(resp, json);
    respondJson(client, json, 400);
    return;
  }

  Serial.println(F("/api/ftp-discover-fingerprint: start"));
  const unsigned long t0 = millis();
  gFtpBackupCriticalPathActive = true;
  gBackupInProgress = true;
  setFtpBackupStage("fp-discover-start");
  ftpsTraceReset();
  serviceTransferWatchdog();
  gWebServer.end();   // free a LISTEN PCB so the data socket has headroom

  NetworkInterface *network = Ethernet.getNetwork();
  char fingerprintBuf[80];
  fingerprintBuf[0] = '\0';
  char errBuf[160];
  errBuf[0] = '\0';
  bool ok = false;
  ...
  if (network == nullptr) {
    snprintf(errBuf, sizeof(errBuf), "Ethernet not initialized");
  } else if (!gFtpsClient.begin(network, errBuf, sizeof(errBuf))) {
    // begin() may report its own error; fall through to surface it
  } else {
    gFtpsClient.setTraceCallback(ftpsTraceCallback);
    setFtpsTransportTraceHook(ftpsTraceCallback);
    setFtpsClientProgressHook(ftpsProgressKick);
    setFtpBackupStage("fp-discover-handshake");
    serviceTransferWatchdog();
    ok = gFtpsClient.discoverFingerprint(host, port, sni,
                                         fingerprintBuf, sizeof(fingerprintBuf),
  ...
  gBackupInProgress = false;
  gFtpBackupCriticalPathActive = false;
  setFtpBackupStage("fp-discover-done");

  resp["ok"] = ok;
  resp["elapsedMs"] = (uint32_t)(millis() - t0);
  if (ok) {
    resp["fingerprint"] = fingerprintBuf;
  } else {
    resp["error"] = (errBuf[0] != '\0') ? errBuf : "Fingerprint discovery failed";
  }
  resp["trace"] = gFtpsTraceBuf;

  Serial.print(F("/api/ftp-discover-fingerprint: done ok="));
  Serial.print(ok ? 1 : 0);
  Serial.print(F(" elapsed_ms="));
...
static bool sendConfigViaNotecard(const char *clientUid, const char *jsonPayload) {
  ...
  // Inject config creation timestamp so client can reject old out-of-order configs
  ClientConfigSnapshot *snap = findClientConfigSnapshot(clientUid);
  if (snap) {
    if (snap->configVersion[0] != '\0') {
...
static void handleDaily(JsonDocument &doc, double epoch) {
  ...
  for (JsonObject t : sensors) {
    ...
    // Prefer per-sensor acquisition epoch (client v2.0.53+); fall back to top-level
    // report epoch, then server-now. Resolves the orphaned-epoch bug where a stale
    // cached reading (ru/sf flagged) used to get bumped to report-time, masking the
    // true age of the last successful acquisition on the dashboard.
...
static void sendDailyEmail() {
  ...
  for (uint8_t i = 0; i < gSensorRecordCount; ++i) {
    ...
    // Outbound JSON key kept as "levelInches" for Notehub→SMTP route template compatibility,
...
static void publishViewerSummary() {
  ...
  for (uint8_t i = 0; i < gSensorRecordCount; ++i) {
    ...
    obj["l"] = roundTo(gSensorRecords[i].currentValue, 1);
    ...
    // Include 24hr change if available
    if (gSensorRecords[i].previousLevelEpoch > 0.0) {
      obj["d"] = roundTo(gSensorRecords[i].currentValue - gSensorRecords[i].previousValue, 1);
    }
    obj["a"] = gSensorRecords[i].alarmActive;
    obj["at"] = gSensorRecords[i].alarmType;
    obj["u"] = gSensorRecords[i].lastUpdateEpoch;
...
// Look up the preset SG for a fluid name. Returns 1.0 (water) for unknown names.
// Mirror of the table on the client. Keep these in sync.
static float getPresetSpecificGravityServer(const char *fluidName) {
  if (!fluidName) return 1.0f;
  if (strcmp(fluidName, "water") == 0)        return 1.00f;
  if (strcmp(fluidName, "diesel") == 0)       return 0.85f;
  if (strcmp(fluidName, "gasoline") == 0)     return 0.74f;
  if (strcmp(fluidName, "heatingOil") == 0)   return 0.85f;
  if (strcmp(fluidName, "propane") == 0 || strcmp(fluidName, "lpg") == 0) return 0.50f;
  if (strcmp(fluidName, "brine") == 0)        return 1.20f;
  if (strcmp(fluidName, "crudeOil") == 0)     return 0.83f;
  if (strcmp(fluidName, "usedOil") == 0)      return 0.92f;
  if (strcmp(fluidName, "glycol50") == 0)     return 1.07f;
  if (strcmp(fluidName, "def") == 0 || strcmp(fluidName, "adblue") == 0) return 1.09f;
  if (strcmp(fluidName, "ethanol") == 0)      return 0.79f;
  return 1.0f;
}

// After learnedSlope is computed, back out an implied fluid SG from the difference
// between the learned mA->inches slope and the theoretical water slope predicted by
// the sensor's configured pressure range. Stores 0.0f if not derivable (e.g., sensor
// is ultrasonic, gas, missing config, or non-pressure unit).
//
//   theoretical_water_slope = (rangeMax - rangeMin) * pressureFactor / 16   [inches per mA, water]
//   impliedSG               = theoretical_water_slope / learnedSlope
//
// A diesel tank should yield impliedSG ~ 0.85; brine ~ 1.20; etc. Drift from the
// admin's selected fluid preset by > 0.05 likely indicates wrong fluid label, sensor
// drift, or contamination.
static void computeImpliedSpecificGravity(SensorCalibration *cal) {
  if (!cal) return;
  cal->impliedSpecificGravity = 0.0f;
  cal->specificGravityDriftFromPreset = 0.0f;

  if (cal->learnedSlope <= 0.0f) return;
  if (strcmp(cal->sensorType, "pressure") != 0) return;

  ClientConfigSnapshot *snap = findClientConfigSnapshot(cal->clientUid);
  if (!snap || strlen(snap->payload) == 0) return;

  JsonDocument doc;
  if (deserializeJson(doc, snap->payload)) return;
  JsonArray sensors = doc["sensors"];
  if (!sensors) return;

  for (JsonVariant t : sensors) {
    uint8_t tn = t["number"] | 0;
    if (tn != cal->sensorIndex) continue;

    const char *monitorType = t["monitorType"] | "tank";
    if (strcmp(monitorType, "gas") == 0) return;  // SG is meaningless for gas

    float rangeMin = t["sensorRangeMin"] | 0.0f;
    float rangeMax = t["sensorRangeMax"] | 0.0f;
    if (rangeMax <= rangeMin) return;

    const char *rangeUnit = t["sensorRangeUnit"] | "PSI";
    float pressureFactor = 27.68f;
    ...
    float theoreticalWaterSlope = (rangeMax - rangeMin) * pressureFactor / 16.0f;  // inches/mA assuming water
    float impliedSg = theoreticalWaterSlope / cal->learnedSlope;
    if (impliedSg < 0.3f || impliedSg > 2.0f) return;  // implausible -- bad cal data
    cal->impliedSpecificGravity = impliedSg;

    // Compare to the admin-selected fluid preset (or override) for drift warning.
    float presetSg = t["fluidSpecificGravity"] | 0.0f;
    if (presetSg < 0.3f || presetSg > 2.0f) {
      const char *fluidTypeName = t["fluidType"] | "water";
      presetSg = getPresetSpecificGravityServer(fluidTypeName);
    }
    cal->specificGravityDriftFromPreset = fabs(impliedSg - presetSg);
    return;
  }
}
...
// (legacy theoretical path) was removed in the v1.6.15 conversion architecture restoration:
// the client now ships a self-describing "lvl", and resolveLevel() reconciles it against the
// server's learned calibration. See resolveLevel() below.
    ...
    for (JsonObject obj : arr) {
      ...
      if (existing) {
        if (loadedEpoch > existing->lastUpdateEpoch) {
          ...
          existing->currentValue = obj["l"] | 0.0f;
          ...
          existing->previousValue = obj["pl"] | 0.0f;
...
static void checkStaleClients() {
  ...
  for (uint8_t i = 0; i < gClientMetadataCount; ++i) {
    ...
    if (offlineSeconds >= STALE_CLIENT_THRESHOLD_SECONDS) {
      ...
      // v1.7.1: requires operator approval (Approve Deletion in the dashboard) so a long
      // offline period — e.g. a seasonal site or a comms outage — never silently destroys data.
  ...
  // Persist metadata when a stale-deletion flag was set or cleared above.
  if (gClientMetadataDirty) {
    saveClientMetadataCache();
    gClientMetadataDirty = false;
  }
...
static void saveClientConfigSnapshots() {
    ...
    String output;
    output.reserve(gClientConfigCount * CLIENT_CONFIG_CACHE_LINE_MAX);
    for (uint8_t i = 0; i < gClientConfigCount; ++i) {
      // Format: uid\tpayload\tpendingDispatch\tconfigVersion\tlastAckEpoch\tlastAckStatus\tdispatchAttempts\tlastDispatchEpoch
...
static void sendHistoryJson(EthernetClient &client, const String &query) {
  ...
  for (uint8_t h = 0; h < gSensorHistoryCount; h++) {
    ...
    sensorObj["sensorIndex"] = hist.sensorIndex;
    // Resolve the human-readable label from the live sensor registry instead of
    // hardcoding "Sensor". Without this, every chart series legend reads identically
    // ("Site - Sensor"), making it impossible to tell tanks apart on the level chart.
    const char *resolvedLabel = "Sensor";
    for (uint8_t r = 0; r < gSensorRecordCount; ++r) {
      if (strcmp(gSensorRecords[r].clientUid, hist.clientUid) == 0 &&
          gSensorRecords[r].sensorIndex == hist.sensorIndex &&
          gSensorRecords[r].label[0] != '\0') {
        resolvedLabel = gSensorRecords[r].label;
        break;
      }
    }
    sensorObj["label"] = resolvedLabel;
    sensorObj["heightInches"] = hist.heightInches;
    ...
    for (uint16_t j = 0; j < hist.snapshotCount; j++) {
      ...
      // Defensive: drop any snapshot that lacks a valid timestamp. Older builds
      // could record timestamp=0 when the server clock had not synced yet; those
      // points used to silently disappear from the chart anyway because of the
      // cutoff filter below, but leaving them in the payload wastes bytes and
      // confuses downstream consumers (e.g. CSV export).
      if (snap.timestamp <= 0.0) continue;
...
static void handleArchivedClients(EthernetClient &client, const String &query) {
  ...
  if (fileParam.length() > 0) {
    ...
    // used by archiveClientToFtp() that wrote the file. Some FTPS-only servers
    // refuse plain-FTP retrieval entirely, so the protocols must be paired.
    const bool useFtps = gConfig.ftpsEnabled;
    FtpSession session;
    char err[128];
    bool connected = useFtps
        ? ftpsConnectAndLogin(err, sizeof(err))
        : ftpConnectAndLogin(session, err, sizeof(err));
    if (!connected) {
    ...
    size_t bufLen = 0;
    bool ok = useFtps
        ? ftpsRetrieveBuffer(fileParam.c_str(), buf, sizeof(buf), bufLen, err, sizeof(err))
        : ftpRetrieveBuffer(session, fileParam.c_str(), buf, sizeof(buf), bufLen, err, sizeof(err));
    if (useFtps) {
      ftpsQuit();
    } else {
      ftpQuit(session);
...
static void handleServerSettingsPost(EthernetClient &client, const String &body) {
  ...
  // Update GitHub Route Alias (Notehub route used to proxy GitHub API calls)
...
static void recalculateCalibration(SensorCalibration *cal) {
  ...
  if (useTempCompensation) {
    ...
    if (fabs(det) < 0.0001f) {
      ...
      computeImpliedSpecificGravity(cal);
...
static void handleCalibrationGet(EthernetClient &client) {
  ...
  for (uint8_t i = 0; i < gSensorCalibrationCount; ++i) {
    ...
    // Implied SG (back-derived from learnedSlope) and drift from admin's fluid preset.
    // 0.0 means "not derivable" (e.g., gas monitor, ultrasonic, or missing config).
    obj["impliedSpecificGravity"] = cal.impliedSpecificGravity;
    obj["sgDriftFromPreset"] = cal.specificGravityDriftFromPreset;
  }
...
static void handleDebugSensors(EthernetClient &client, const String &method, const String &body, const String &queryString) {
  ...
  for (uint8_t i = 0; i < gSensorRecordCount; ++i) {
    ...
    responseStr += ",\"l\":" + String(rec.currentValue, 3);
